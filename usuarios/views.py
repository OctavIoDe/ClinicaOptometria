...
class IniciarSesion(LoginView):
    ...
    def get_success_url(self):
        usuario = self.request.user.nombre
        ...
        if fecha_registro == ultima_vez:
            messages.success(
                self.request,
                f'Hola {usuario}! que tengas un exelente primer dia en el sistema!'
            )
        else:
            messages.success(
                self.request,
                f'Hola {usuario}! Ultimo incio de sesión el: {ultima_vez}'
            )
    ...
    def get_context_data(self, **kwargs):
        gcd = super().get_context_data(**kwargs)
        gcd['titulo'] = 'Iniciar Sesión'
        return gcd
...
class ListaActividades(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        gcd = super().get_context_data(**kwargs)
        gcd['titulo'] = 'Registro de actividades'
        gcd['buscar'] = 'Nombre de usuario o actividad'
        return gcd
...
class ListaUsuarios(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        gcd = super().get_context_data(**kwargs)
        gcd['titulo'] = 'Lista de usuarios'
        gcd['buscar'] = 'Nombre de usuario o sector'
        return gcd